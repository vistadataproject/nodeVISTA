...
#
# VDP changed for simpler setup (and comments)
# 
#---------------------------------------------------------------------------
...
# VDP: vagrant sets to true
if [[ -z $installEWD ]]; then
    installEWD=false
fi

# VDP: instance not set in Vagrant => set here
if [[ -z $instance ]]; then
    instance=osehra
fi
...
# just for Vagrant
    ...
    echo "no vagrant directory specified - exiting"
    exit 1
fi

# bootstrap the system
cd $scriptdir
# VDP: changing to have this in the scriptdir/vagrant dir
./bootstrapUbuntuServer.sh

# Ensure scripts know if we are RHEL like or Ubuntu like
export ubuntu=true;

# Install GTM
# VDP: need to specify version as this will be the subdirectory and appear in all /etc/env gtm_dist etc settings
cd GTM
...
if $skipTests; then
    ...
    echo "NOT ALLOWING TEST INSTALL IN THIS FIX UP"
    exit 1
