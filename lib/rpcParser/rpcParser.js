...
/*
 * Must reproduce the functionality of XWBPRS.m, PRS5
 */
        ...
        for (var paramnum = 0; paramnum < rpcObjectInputParameters.length; paramnum++) {
            if (rpcObjectInputParameters[paramnum].parameterType === 'LIST') {
                ...
                for (var item = 0; item < parameterList.length; item++) {
                    ...
                    // and that value is an integer - latter is needed to a/c for ORWDX SAVE's
                    // 'WP' argument
                    if ((parameterList[item].key.slice(-2) === ',0') && (Number.isInteger(parameterList[item].value))) {
                        ...
                        if (subarray.length === 0)
                            throw "0 length Array - not allowed";
